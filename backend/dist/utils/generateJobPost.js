...
exports.generateJobPost = generateJobPost;
const generativeAi_config_1 = require("../config/generativeAi.config");
async function generateJobPost(title, categories, subcategories) {
    try {
        const prompt = `
        ...
        `;
        const result = await generativeAi_config_1.genAiModel.generateContent(prompt);
